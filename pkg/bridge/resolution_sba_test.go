...
func TestResolution_DealsLethalDamage_KillsCreature_AndAuraDetaches(t *testing.T) {
    ...
    if err := sb.Stack().ResolveTop(); err != nil { t.Fatalf("resolve error: %v", err) }

    // SBAs should have moved the creature to graveyard
    if len(p1.Battlefield) != 0 { t.Fatalf("expected battlefield empty, got %d", len(p1.Battlefield)) }
    if len(p1.Graveyard) != 2 { t.Fatalf("expected both creature and aura in graveyard, got %d", len(p1.Graveyard)) }
}
...
func TestResolution_PlayerLifeReachesZero_PlayerLoses(t *testing.T) {
    ...
    if !p2.HasLost() { t.Fatalf("expected P2 to have lost after life <= 0") }
}

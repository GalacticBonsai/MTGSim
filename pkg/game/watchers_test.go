...
func TestWatcher_CreatureETBCountsAndResets(t *testing.T) {
	...
	if _, err := g.SummonCreature(p1, c1.Name); err != nil {
		t.Fatalf("summon 1: %v", err)
	}
	if _, err := g.SummonCreature(p1, c2.Name); err != nil {
		t.Fatalf("summon 2: %v", err)
	}

	if w.Count != 2 {
		t.Fatalf("expected watcher count 2, got %d", w.Count)
	}
	...
	if w.Count != 0 {
		t.Fatalf("expected watcher reset to 0, got %d", w.Count)
	}
}
